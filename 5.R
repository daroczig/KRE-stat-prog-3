...
## check flight number: does that summary look OK?
## check on H2O web interface as well (enum vs factor)
## convert to enum using the UI then check summary again
summary(hflights.hex)
## now it shows a frequency table

## convert numeric to factor/enum using R
hflights.hex['FlightNum'] <- as.factor(hflights.hex['FlightNum'])
summary(hflights.hex)
## NOTE that the above might not update the "hlights" frame in the UI
## but instead creates a new "RTMP" frame, but the `hflights.hex` reference
## also got updated to that, so now rerunning "summary" should work OK

## let's batch update a few extra columns to factor/enum
...
## let's categorize "day of the hour" into 4 hr intervals
hflights[, hour := cut(as.numeric(hour), seq(0, 24, 4))]
hflights[, .N, by = hour]

hflights[is.na(hour)]
hflights[, .N, by = .(is.na(hour), Cancelled == 1)]
## oh right, there's no departure time for cancelled flights

## drop columns leaking information on the cancelled status
...
## build the first random forest model
...
## we should try adding more trees
hflights.rf <- h2o.randomForest(
