...
So that later on no warnings etc are shown:

```{r}
pander(chisq.test(table(mtcars$gear, mtcars$am)))
```
...
And some dynamic stuff:

```{r}
library(shiny)
inputPanel(
    sliderInput('n', label = 'N', min = 100, max = 100000, value = 100, step = 100),
...
```{r}
mycars <- mtcars
for (v in c('am', 'gear', 'carb', 'vs')) {
    mycars[, v] <- factor(mycars[, v])
}
library(GGally)
ggpairs(mycars)
```
